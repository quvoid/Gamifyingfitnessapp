...
class LateralRaiseTracker:
    ...
    def count_lateral_raises(self, coords):
        """Count the number of lateral raises performed."""
        if all(point in coords for point in [
            ...
            # Define raise thresholds
            if angle < 30:  # Arms raised above shoulder level
                self.is_raising = True
            elif self.is_raising and angle > 150:  # Arms returning to sides
                self.lateral_raise_count += 1
                self.is_raising = False

    def draw_count_annotations(self, image):
        """Display the lateral raise count on the image."""
        cv2.putText(image, f'Lateral Raises: {self.lateral_raise_count}', 
    ...
    def process_frame(self, frame):
        ...
        if results.pose_landmarks:
            ...
            # Count lateral raises
            self.count_lateral_raises(coords)
    ...
    def run(self):
        ...
        try:
            while cap.isOpened():
                ...
                # Display the frame
                cv2.imshow('Lateral Raise Tracker', annotated_frame)
